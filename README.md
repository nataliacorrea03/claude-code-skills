...
| **meta-prompt** | Builds ready-to-paste prompts for Meta AI (Muse Spark), which can read public Instagram, Facebook, and Threads directly in ways Claude can't. Describe a research goal (find micro-influencers, videos to react to, brand mentions, niche trends, competitor teardowns, a creator's voice before you pitch them), answer a couple of questions, and get a copy-paste prompt for the meta.ai app. Every prompt self-tests so the model admits what it can't pull instead of faking it. Never fetches data itself, never invents capabilities. | `/meta-prompt` |
